...
| `miniapp/` | WeChat Mini Program (native + Skyline + TypeScript) | `pages/` (login WebView, plus Skyline pages: today, training, analysis, goal, me, history, settings, science, labs, legal), `components/` (`nav-bar` custom Skyline header, `line-chart` Canvas 2D), `utils/` (`api-client.ts` wx.request + JWT, `auth.ts` wx.login flow, `format.ts`/`share.ts`/`theme.ts`), `types/api.ts` auto-synced from `web/src/types/api.ts` by `scripts/sync-types.cjs`, and the EULA + Privacy text (`utils/legal.ts`) from `web/src/lib/legal.ts` by `scripts/sync-legal.cjs` (both run on `npm run typecheck`). The five primary tabs are Today, Training, Analysis, Goal, and Me; Activities lives inside Analysis, while Settings, Science, Labs, and legal surfaces live under Me. The `legal` page renders those Terms/Privacy docs in-app and the login page links to them + gates the waitlist / account-link email capture behind an explicit consent checkbox (WeChat audit requirement). Build: WeChat DevTools handles TypeScript + Sass via `project.config.json` `useCompilerPlugins` — no webpack/babel toolchain. Open `miniapp/` as the DevTools project root; only devDeps are `miniprogram-api-typings` + `typescript` for `tsc --noEmit` CI checks. **Mini program position**: a view + manage companion to the web app. The mini program **owns the waitlist signup** as a CN-friendly entry point — WeChat is the primary discovery channel for that audience and the waitlist is just intent capture (email + optional note), no platform setup. **Full registration + the platform-connection wizard stays on praxys.run** because Garmin / Stryd / Oura OAuth is painful inside a miniapp WebView. Existing users link their account by email / password and then use the mini program day-to-day (signal, managed training, analysis and activities, goal, sync, theory, training-base, theme / language). New users either join the waitlist inside WeChat or, if they already have an invitation code, are deep-linked to praxys.run to register. |
...
- **Garmin OAuth tokens must stay encrypted and generation-fenced.** Garminconnect's `Client.dumps()` bundle is envelope-encrypted on `user_connections`; never pass a persistent filesystem path to `Garmin.login()` or call `Client.dump()`. The credential-generation check prevents an old authenticated session from being attached to replacement credentials.
...
- **Feature gates** are temporary, per-user rollout or eligibility controls for unreleased or risky behavior. Name them in descriptive `lower_snake_case`, default them off, and remove them after rollout. Never put existing generally available behavior behind a default-off positive gate. A GA emergency control must use explicit negative kill-switch semantics, default false, and have a documented justification.
- **Dynamic configs** are runtime-tunable, non-secret values with typed validation and a code-owned fallback. They are not durable user or product truth.
- **Experiments** require an explicit hypothesis, primary and guardrail metrics, assignment unit, exposure point, sample-size/duration plan, and decision owner. Do not create one merely because Statsig exists or when the eligible population is too small to support a decision.
- **Environment variables** remain deployment-wide infrastructure settings, credentials/secrets, hard prerequisites, and safe fallbacks. **Database state** remains durable consent, authorization, user choice, and product state. Statsig never replaces either layer; rollout eligibility must never be treated as a durable execution-target selection or user decision.
- Backend enforcement is authoritative and must run at the authenticated per-user action path; web and miniapp mirror that result in their UX. Use `api/statsig_client.py` and `StatsigContext` rather than scattering raw SDK calls. A client gate is never a security boundary.
- Missing keys and initialization/evaluation errors fail closed: gates return `false`, dynamic configs keep their env/code fallback, and the browser makes no Statsig request without its public client key. Backend identity includes `user_id`, `email`, `is_admin`, `is_demo`, `training_base`, and `language`; browser identity updates safely with authentication.
- Tests cover missing-key defaults, eligible/ineligible paths, identity attributes, typed dynamic-config override/fallback, authoritative runtime enforcement, and affected API/UI behavior. Update env examples, owned deploy workflows, and `docs/ops/config-and-secrets.md` for each key, gate, or config; never expose a server SDK key to a client.
- Preserve web/miniapp capability parity. Miniapp may consume backend-authoritative eligibility without embedding an SDK. If parity cannot ship together, file and link a clearly labelled `miniapp parity gap` follow-up.
- Use the repository `statsig` MCP entry for official Console automation. Each operator or agent authenticates through OAuth; the Statsig organization owner must enable personal Console API keys. Never commit OAuth tokens or Console API keys. Reconcile the declared gates/configs idempotently and reviewably, update operations docs with every resource change, and delete a resource only after verifying its code references are gone.
...
- Web is the canonical type source: `web/src/types/api.ts` → synced to `miniapp/types/api.ts` by `miniapp/scripts/sync-types.cjs` (runs on `npm run typecheck`). EULA + Privacy prose is likewise web-canonical: `web/src/lib/legal.ts` → `miniapp/utils/legal.ts` by `miniapp/scripts/sync-legal.cjs`.
- i18n catalogs are also web-canonical: lingui `.po` files in `web/src/locales/` → consumed by `miniapp/scripts/sync-i18n.cjs`. Add new translatable strings on the web side (or directly to a page's `t(...)` calls) — they propagate to the mini program at typecheck. Genuinely mini-only strings (login copy, modals, tooltips) live in `miniapp/utils/i18n-extra.ts`; if a key also exists on web, CI requires the Chinese value to match instead of silently overriding it.
- **i18n coverage and Chinese quality are enforced**: `miniapp/scripts/check-i18n.cjs` runs on `npm run typecheck` and fails CI if any WXML body / user-visible attribute / `t(...)` key has no zh entry, a hardcoded English-looking literal sneaks into a TS file, placeholders drift, or mini-only Chinese violates the shared voice/terminology rules in `scripts/i18n_glossary.yaml`. Wrap user-visible copy in `t()` / `tFmt()`; for mini-only strings add a `'key': '中文'` pair to `i18n-extra.ts` (both `en` and `zh` blocks). Same-line `// i18n-allow` opts an intentional literal out. Write natural Mainland product Chinese: omit second-person pronouns when possible and use `你`, not `您`, when needed.
