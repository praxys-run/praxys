...
/** A missing HTTP response cannot prove whether a feedback write committed. */
export function feedbackSubmissionTransportUnknown(
  error: { status?: number | null },
): boolean {
  return error.status == null || error.status === 0 || error.status >= 500;
}
...
export function getRecentFeedbackId(): number | null {
  try {
    const value = wx.getStorageSync<unknown>(RECENT_FEEDBACK_ID_KEY);
    const parsed = parseRecentFeedbackId(value);
    if (parsed === null) {
      wx.removeStorageSync(RECENT_FEEDBACK_ID_KEY);
    }
    return parsed;
  } catch {
    return null;
...
export function setRecentFeedbackId(value: unknown): boolean {
  ...
  try {
    wx.setStorageSync(RECENT_FEEDBACK_ID_KEY, String(parsed));
    return true;
  } catch {
