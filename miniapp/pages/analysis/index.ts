...
function timeAgo(isoDate: string, locale: 'en' | 'zh'): string {
  try {
    ...
    console.warn('[analysis] timeAgo failed; dropping stamp:', isoDate, e);
    return '';
  }
}
...
interface TrainingState {
  ...
  activeSection: ObservedTrainingSection;
  loading: boolean;
  errorMessage: string;
  hasResponse: boolean;
  hasAnyData: boolean;
...
function consumeHeatHistoryScrollRequest(): boolean {
  try {
    ...
    // eslint-disable-next-line no-console
    console.warn('[analysis] could not consume heat-history scroll request:', e);
    return false;
  }
}
...
Page<TrainingState & { tr: ReturnType<typeof buildTrainingTr> }, PageMethods>({
  ...
  onShow() {
    ...
    if (curLocale !== pgMut._locale) {
      ...
      localeChanged = true;
      this.setData({
        languageClass: curLocale === 'en' ? 'lang-en' : 'lang-zh',
        tr: buildTrainingTr(),
      });
    }
  ...
  onShareAppMessage() {
    ...
    if (tsb && vol && tsb.value !== '—' && vol.value !== '—') {
      const lead = locale === 'zh' ? '训练分析' : 'Analysis';
      return buildShareMessage(
        `${lead}: TSB ${tsb.value} · ${vol.value} km/wk`,
        '/pages/analysis/index',
      );
    }
    return getShareMessage(locale, '/pages/analysis/index');
  },
  ...
  onScrollToBottom() {
    if (this.data.activeSection !== 'activities') return;
  ...
  onObservedSectionChange(
    ...
    this.setData({
      ...
      metricSheetTitle: '',
      metricSheetDescription: '',
      scrollIntoView: `${value}-section-start`,
    }, () => {
      wx.nextTick(() => this.setData({ scrollIntoView: '' }));
    });
  },
  ...
  scrollToHeatIfPending() {
    ...
    this.setData({ activeSection: 'analysis', scrollIntoView: '' }, () => {
  ...
  async refetch(options?: { background?: boolean }) {
    ...
    try {
      ...
      if (background) {
        // eslint-disable-next-line no-console
        console.warn('[analysis] background refresh failed; keeping cached response:', detail);
        return;
      }
