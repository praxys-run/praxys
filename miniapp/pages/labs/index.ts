...
function buildLabsTr() {
  return {
    ...
    availableDetail: t('Your descriptive stability checks passed, but the chronological test did not show better future-run prediction. Read the curve only as a pattern in eligible past runs.'),
...
Page({
  ...
  async refetch() {
    ...
    try {
      const state = await apiGet<LabsEnvironmentResponseState>('/api/labs/environment-response');
      this.setData({ loading: false, ...deriveState(state) });
  ...
  async onJoin() {
    if (!this.data.adultAttested || !this.data.consentConfirmed) return;
