...
const ROAD_10K_COPY_PARTIAL: Partial<Record<Road10KCopyKey, { en: string; 'zh-CN': string }>> = {
  ...
  'success.withdrawn': { en: 'You left the Road 10K rollout. Your proposal is read-only and cannot be adopted or regenerated. Any adopted plan stays in Training and is not paused or ended. Export and account deletion remain available. Rollout data follows the current notice.', 'zh-CN': '你已退出公路 10K 试点。你的提案为只读，无法采纳或重新生成。任何已采纳计划都会保留在训练中，不会暂停或结束。你仍可导出数据或删除账户。试点数据按当前说明处理。' },
