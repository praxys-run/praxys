...
def load_environment_response_preflight_counts(
    ...
    *,
    ...
    statement = text(
        ...
        "    block.block_id, "
        "    MAX(block.block_start_sec, MIN("
        "      hr.interval_end_sec - :minimum_segment_duration_sec, "
        "      block.block_end_sec - :minimum_segment_duration_sec"
        "    )) AS window_start_sec "
        "  FROM eligible_power_blocks AS block "
        "  JOIN heart_rate_intervals AS hr "
        ...
        "  SELECT window.activity_id, window.stream_source, window.kind, "
        "    window.block_id, window.window_start_sec, "
        "    hr.stream_source AS heart_rate_source, "
        "    COALESCE(SUM("
        "      MIN("
        "        window.window_start_sec + :minimum_segment_duration_sec, "
        "        hr.interval_end_sec"
        "      ) - MAX(window.window_start_sec, hr.interval_start_sec)"
        "    ), 0) AS heart_rate_overlap_sec "
        "  FROM power_window_starts AS window "
        "  LEFT JOIN heart_rate_intervals AS hr "
        "    ON hr.activity_id = window.activity_id "
        "    AND hr.interval_end_sec > window.window_start_sec "
        "    AND hr.interval_start_sec "
        "      < window.window_start_sec + :minimum_segment_duration_sec "
        "  GROUP BY window.activity_id, window.stream_source, window.kind, "
        "    window.block_id, window.window_start_sec, hr.stream_source"
        "), sample_summary AS ("
        ...
        "        window.heart_rate_overlap_sec "
        "          >= :minimum_heart_rate_duration_sec "
        "        THEN 1 ELSE 0 END) "
        "      FROM power_window_coverage AS window "
        "      WHERE window.activity_id = activities.activity_id "
        "        AND window.kind = 'power'), 0) "
