...
def load_environment_response_preflight_counts(
    ...
    *,
    ...
    """Load definite, aggregate-only blockers for the Labs preflight.

    The preflight intentionally checks broad sample presence and span rather
    than rebuilding the full analysis's contiguous-window segmentation. Exact
    power/heart-rate continuity is confirmed by the queued full analysis.
    """
    statement = text(
        "WITH eligible AS ("
        ...
        "), sample_summary AS ("
        "  SELECT sample.activity_id, "
        "    MAX(CASE WHEN sample.power_watts > 0 "
        "      AND sample.power_watts <= :maximum_power_watts "
        "      THEN sample.t_sec END) "
        "      - MIN(CASE WHEN sample.power_watts > 0 "
        "        AND sample.power_watts <= :maximum_power_watts "
        "        THEN sample.t_sec END) AS power_span_sec, "
        "    MAX(CASE WHEN LOWER(sample.source) = 'stryd' "
        "      AND sample.power_watts > 0 "
        "      AND sample.power_watts <= :maximum_power_watts "
        "      THEN sample.t_sec END) "
        "      - MIN(CASE WHEN LOWER(sample.source) = 'stryd' "
        "        AND sample.power_watts > 0 "
        "        AND sample.power_watts <= :maximum_power_watts "
        "        THEN sample.t_sec END) AS stryd_power_span_sec, "
        "    MAX(CASE WHEN sample.hr_bpm BETWEEN 30 AND 240 "
        "      THEN sample.t_sec END) "
        "      - MIN(CASE WHEN sample.hr_bpm BETWEEN 30 AND 240 "
        "        THEN sample.t_sec END) AS heart_rate_span_sec "
        "  FROM activity_samples AS sample "
        "  JOIN eligible "
        "    ON eligible.activity_id = sample.activity_id "
        "  WHERE sample.user_id = :uid "
        "  GROUP BY sample.activity_id"
        ") "
        "SELECT "
        ...
        "  SUM(CASE WHEN COALESCE(power_span_sec, 0) "
        "    >= :minimum_power_span_sec "
        "    THEN 1 ELSE 0 END) AS power_activity_count, "
        "  SUM(CASE WHEN COALESCE(heart_rate_span_sec, 0) "
        "    >= :minimum_heart_rate_span_sec "
        "    THEN 1 ELSE 0 END) AS heart_rate_activity_count, "
        "  SUM(CASE WHEN temperature_c IS NOT NULL "
        "    AND relative_humidity_pct IS NOT NULL "
        "    AND COALESCE(power_span_sec, 0) "
        "      >= :minimum_power_span_sec "
        "    AND COALESCE(heart_rate_span_sec, 0) "
        "      >= :minimum_heart_rate_span_sec "
        "    THEN 1 ELSE 0 END) AS complete_any_provider_activity_count, "
        "  SUM(CASE WHEN COALESCE(stryd_power_span_sec, 0) "
        "    >= :minimum_power_span_sec "
        "    THEN 1 ELSE 0 END) AS stryd_power_activity_count, "
        "  SUM(CASE WHEN temperature_c IS NOT NULL "
        "    AND relative_humidity_pct IS NOT NULL "
        "    AND COALESCE(stryd_power_span_sec, 0) "
        "      >= :minimum_power_span_sec "
        "    AND COALESCE(heart_rate_span_sec, 0) "
        "      >= :minimum_heart_rate_span_sec "
    ...
    row = db.execute(
        statement,
        {
            ...
            # The full interval calculation may credit one final valid sample
            # interval. Subtract it here so a quick preflight cannot reject a
            # boundary case that the authoritative analysis would accept.
            "minimum_power_span_sec": max(
                0.0,
                minimum_segment_duration_sec - maximum_sample_interval_sec,
            ),
            "minimum_heart_rate_span_sec": max(
                0.0,
                (
