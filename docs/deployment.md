...
- **Pre-merge backend and frontend quality** (`.github/workflows/ci-backend.yml`) — runs on every `pull_request` to `main`. `python-tests` runs the backend suite; `frontend-quality` combines the production web build with the Impeccable/evidence UI gate. The existing required `backend-tests` context remains a compatibility umbrella until branch protection also requires `frontend-quality`, so either domain still blocks merge during the transition. No `paths:` filter on purpose: a required check skipped by a path filter stays permanently pending and blocks the PR.
