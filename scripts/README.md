...
| [`garmin_mfa_probe.py`](./garmin_mfa_probe.py) | Probe Garmin MFA strategy token acceptance without writing a tokenstore | When validating portal/widget behavior after an upstream authentication change. |
...
| [`translate_missing.py`](./translate_missing.py) | Translate new copy and review existing Lingui `.po` entries with page context via Azure AI | `po` writes new strings only after a separate high-confidence semantic check; `review-po` runs a bounded native-language pass over a stable catalog shard. Requires `AZURE_AI_ENDPOINT` and Azure credentials. |
...
- Diagnostic scripts (`garmin_diagnose.py`, `garmin_mfa_probe.py`) do not persist Garmin OAuth tokens.
