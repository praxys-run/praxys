...
def test_shipped_registry_is_valid_and_heat_migration_is_complete() -> None:
    ...
    assert all(not claim.effect_estimates for claim in five_km_review.claims)
    assert all("metadata-only draft" in claim.statement for claim in five_km_review.claims)
    assert any(
